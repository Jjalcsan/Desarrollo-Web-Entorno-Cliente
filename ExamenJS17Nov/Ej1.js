...
const compruebaFormulario = () => {
    ...
    if (valid != -1){
        ...
        localStorage.setItem(document.getElementById("login").value, JSON.stringify(objetoLocal))
...
consultar.addEventListener("click", () => {
    ...
    for(let i = 0; i<localStorage.length; i++){
        if( usuario == localStorage.key(i)){
            div = document.getElementById("div-para-mostrar-usuarios")
            lista = localStorage.getItem(usuario).split(",");
            div.innerHTML=`<table><tr><td>${lista[0]}</td></tr><tr><td>${lista[1]}</td></tr><tr><td>${lista[2]}</td></tr><tr><td>${lista[3]}</td></tr><tr><td>${lista[4]}</td></tr><tr><td>${lista[5]}</td></tr></table>`
        }
    }
})